...
import { setMessageSent } from "@/lib/features/chatSlice"
...
export const MessageForm: React.FC = () => {
  ...
  const chatUserData = useSelector((state: RootState) => state.chatReducer)
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      const { data } = await axiosInstance.post("/user/sendMessage", { author: userData.username, body: values.message, sendTo: chatUserData.username })
      ...
      <Form {...form} >
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 md:space-y-6">
          <div className="flex flex-row ">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl >
                      <Input placeholder="" {...field} {...form.register} className="dark:bg-gray-900 bg-gray-300 rounded-t-none focus:outline-none" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
            ...
            <button className="items-center flex max-h-10 mr-1 rounded px-3 py-2 hover:bg-gray-900 hover:dark:bg-gray-900 bg-gray-600 dark:bg-gray-800 shadow ">
